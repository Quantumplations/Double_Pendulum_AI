...
def main():
    print("Loading Trained GNN Model & Scalers...")
    ...
    print("2. Generating AI Rollout via Autoregressive Simulation with GNN...")
    gnn_trajectory = np.zeros((steps, 4))
    gnn_trajectory[0] = y0
    ...
    axs[0].plot(t_eval, gnn_trajectory[:, 0], 'r--', label='Predicted Theta 1 (GNN)')
    axs[0].set_ylabel('Theta 1 (rad)')
    axs[0].legend()
    axs[0].grid(True)
    axs[0].set_title(f'GNN Rollout vs Physics over {trajectory_seconds}s')
    
    # Theta 2
    axs[1].plot(t_eval, true_trajectory[:, 1], 'g-', label='True Theta 2 (Physics)')
    axs[1].plot(t_eval, gnn_trajectory[:, 1], 'm--', label='Predicted Theta 2 (GNN)')
    ...
    plt.tight_layout()
    plt.savefig('rollout_comparison_gnn.png')
    print("Saved plot to rollout_comparison_gnn.png")
